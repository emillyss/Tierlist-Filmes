...
export async function createSessionToken(userId: string, userEmail: string){
    ...
    .sign(encodedKey); //Assina o token(combina o segredo com as credenciais usando o algoritmo acima)

    //Seguindo a documentação do next, precisamos primeiro criar o cookieStore, pois é async
    const cookieStore = await cookies();
    
    //guarda o token no cookie
...
export async function isSessionValid(){
    ...
    if(sessionCookie)
    {
        ...
        const session = await openSessionToken(value); //abre para verificar se é a pessoa
        return session;
    }
